...
import { CheckCircle2, AlertTriangle, FileSpreadsheet, RefreshCw, Layers } from 'lucide-react';
      ...
      <div className="space-y-4 text-xs sm:text-sm">
        ...
        <div className="p-3.5 rounded-xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200/60 dark:border-amber-800/60 text-amber-800 dark:text-amber-300 text-xs flex items-start gap-2.5">
          ...
          <div className="space-y-1">
            <span className="font-bold">Atomic Replacement Policy:</span>
            <p className="leading-relaxed">
              Confirming this synchronization will replace the active allowed-user whitelist with the verified emails below.
              Existing registered student profiles and historical registration data will remain completely intact.
            </p>
          </div>
        </div>
        ...
        {result.invalidRows.length > 0 && (
          <div className="space-y-1.5 pt-1">
            <h4 className="font-display font-bold text-rose-700 dark:text-rose-400 flex items-center gap-1.5 text-xs">
              <AlertTriangle className="w-3.5 h-3.5" />
              <span>Invalid Row Details (Excluded from whitelist)</span>
            </h4>
            <div className="max-h-36 overflow-y-auto border border-rose-200/60 dark:border-rose-900/60 rounded-xl p-2.5 bg-rose-50/50 dark:bg-rose-950/20 text-[11px] divide-y divide-rose-200/40 dark:divide-rose-900/40">
              {result.invalidRows.map((inv, i) => (
                <div key={i} className="py-1.5 flex items-center justify-between">
                  <span className="font-mono text-kaziranga-800 dark:text-cream-200">Row {inv.row}: &quot;{inv.email}&quot;</span>
                  <span className="text-rose-600 dark:text-rose-400 font-semibold">{inv.reason}</span>
                </div>
              ))}
            </div>
          </div>
