...
const AuthSplash: React.FC<{ message: string }> = ({ message }) => (
  <div className="fixed inset-0 z-50 ed-stage ed-mesh ed-grain flex items-center justify-center p-4">
    <div
      className="relative z-[2] flex flex-col items-center gap-5 px-8 py-10 sm:px-12
        rounded-3xl bg-white/[0.04] backdrop-blur-xl border border-white/10 shadow-e-4"
      role="status"
      aria-live="polite"
    >
      ...
      <div className="flex items-center gap-2.5">
        <Loader2
          className="w-4 h-4 animate-spin text-[rgb(var(--accent-vivid))]"
          aria-hidden
        />
        <span className="text-caption font-display font-semibold text-white/70">{message}</span>
      </div>
    </div>
  </div>
);
